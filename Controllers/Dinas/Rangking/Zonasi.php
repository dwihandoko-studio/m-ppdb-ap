...
use App\Models\Dinas\Rangking\ZonasiModel;
...
class Zonasi extends BaseController
{
    ...
    public function getAllSekolah()
    {
        ...
        foreach ($lists as $list) {
            ...
            $action =
                '
                            <a target="_blank" href="' . base_url('dinas/rangking/zonasi/sekolah') . '?token=' . $list->tujuan_sekolah_id_1 . '" class="btn btn-primary btn-sm">
                                <i class="fa fa-eye"></i>
                                <span>Detail</span>
                            </a>';
            $row[] = $action;
    ...
    {
        return redirect()->to(base_url('dinas/rangking/zonasi/data'));
    }

    public function data()
    {
        $data['title'] = 'Rekapitulasi Rangking Zonasi';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->code != 200) {
            delete_cookie('jwt');
        ...
        return view('dinas/rangking/zonasi/sekolah', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('dinas/rangking/zonasi/detail', $data);
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->code = 200;
            $response->result = $oldData;
            $response->data = view('dinas/rangking/zonasi/edit', $data);
            $response->message = "Data ditemukan.";
            return json_encode($response);
        }
    }
