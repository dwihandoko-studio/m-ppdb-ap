    ...
    public function index()
    {
        ...
        if ($cekRegisterApprove) {
            if ($cekRegisterApprove->status_pendaftaran == 2) {
                $data['success'] = "Anda dinyatakan <b>LOLOS</b> pada seleksi PPDB Tahun Ajaran 2023/2024 di <b>" . getNamaAndNpsnSekolah($cekRegisterApprove->tujuan_sekolah_id_1) . "</b> Via Jalur " . $cekRegisterApprove->via_jalur . ". <br/>Selanjutnya silahkan melakukan konfirmasi dan daftar ulang ke Sekolah Tujuan.";
            }
            if ($cekRegisterApprove->status_pendaftaran == 3) {
                if ($cekRegisterApprove->via_jalur == "AFIRMASI") {
                    $data['warning'] = "Anda dinyatakan <b>TIDAK LOLOS</b> seleksi PPDB Tahun Ajaran 2023/2024 di <b>" . getNamaAndNpsnSekolah($cekRegisterApprove->tujuan_sekolah_id_1) . "</b> Via Jalur " . $cekRegisterApprove->via_jalur . ". <br/>Selanjutnya anda dapat melakukan pendaftaran menggunakan jalur yang lain (ZONASI, PRESTASI, MUTASI).";
                } else {
                    $data['warning'] = "Anda dinyatakan <b>TIDAK LOLOS</b> seleksi PPDB Tahun Ajaran 2023/2024 di <b>" . getNamaAndNpsnSekolah($cekRegisterApprove->tujuan_sekolah_id_1) . "</b> Via Jalur " . $cekRegisterApprove->via_jalur . ".";
                }
            }
