        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                try {
                    ...
                    $riwayatLib->insert("Mendaftar Melalui Jalur Afirmasi, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Jalur Afirmasi");
                } catch (\Throwable $th) {
                }
                $response = new \stdClass;
                $response->code = 200;
                $response->data = $data;
                $response->message = "Pendaftaran Melalui Jalur Afirmasi berhasil dilakukan.";
                return json_encode($response);
            } else {
                $this->_db->transRollback();
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Pendaftaran Melalui Jalur Afirmasi gagal dilakukan.";
                return json_encode($response);
            }
