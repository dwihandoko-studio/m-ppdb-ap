        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                try {
                    ...
                    $riwayatLib->insert("Mendaftar Melalui Jalur Mutasi, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Jalur Mutasi");
                } catch (\Throwable $th) {
                }
                $response = new \stdClass;
                $response->code = 200;
                $response->data = $data;
                $response->message = "Pendaftaran Melalui Jalur Mutasi berhasil dilakukan";
                return json_encode($response);
            } else {
                $this->_db->transRollback();
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Pendaftaran Melalui Jalur Mutasi gagal dilakukan.";
                return json_encode($response);
            }
        }
    }
