...
class Swasta extends BaseController
{
    ...
    public function getAll()
    {
        ...
        if ($data['countData'] > 0) {
            if (count($data['result']) > 0) {
                ...
                $response->data = view('peserta/pendaftaran/swasta/pilihan-zonasi', $data);
                $response->pagination = view('peserta/pendaftaran/swasta/pilihan-pagination', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->code = 204;
    ...
    {
        $data['title'] = 'DAFTAR JALUR SWASTA';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->code != 200) {
            delete_cookie('jwt');
        ...
        return view('peserta/pendaftaran/swasta/index', $data);
    }
        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                try {
                    ...
                    $riwayatLib->insert("Mendaftar Melalui Jalur Swasta, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Jalur Swasta");
                } catch (\Throwable $th) {
                }
                $response = new \stdClass;
                $response->code = 200;
                $response->data = $data;
                $response->message = "Pendaftaran Melalui Jalur Swasta berhasil dilakukan.";
                return json_encode($response);
            } else {
                $this->_db->transRollback();
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Pendaftaran Melalui Jalur Swasta gagal dilakukan.";
                return json_encode($response);
            }
        }
    }
