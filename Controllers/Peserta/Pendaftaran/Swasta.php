                    ...
                    if ($decoded) {
                        ...
                        if ($this->_db->affectedRows() > 0) {
                            ...
                            try {
                                ...
                                $riwayatLib->insert("Mendaftar via Jalur Swasta ke Sekolah $name, dengan No Pendaftaran : " . $data['kode_pendaftaran'], "Daftar Jalur Swasta");
                            } catch (\Throwable $th) {
                            }
                            $response = new \stdClass;
                            $response->code = 200;
                            $response->data = $data;
                            $response->message = "Pendaftaran via jalur Swasta ke Sekolah $name berhasil dilakukan. Kode pendaftaran anda : " . $data['kode_pendaftaran'] . ". Selanjutnya Silahkan cetak bukti pendaftaran anda.";
                            return json_encode($response);
                        } else {
                            $this->_db->transRollback();
                            $response = new \stdClass;
                            $response->code = 400;
                            $response->message = "Pendaftaran via jalur Swasta ke Sekolah $name berhasil dilakukan.";
                            return json_encode($response);
                        }
                    } else {
                        delete_cookie('jwt');
