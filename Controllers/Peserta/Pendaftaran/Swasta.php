...
class Swasta extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($canDaftar->code !== 200) {
            $data['jalur'] = "SWASTA";
            $data['message'] = $canDaftar->message . " untuk <b>Sekolah Swasta</b>.";
        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                try {
                    ...
                    $riwayatLib->insert("Mendaftar via Jalur Swasta, untuk diverifikasi berkas oleh sekolah tujuan.", "Daftar Jalur Swasta");
                } catch (\Throwable $th) {
                }
                $response = new \stdClass;
                $response->code = 200;
                $response->data = $data;
                $response->message = "Pendaftaran via jalur Swasta berhasil dilakukan.";
                return json_encode($response);
            } else {
                $this->_db->transRollback();
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Pendaftaran via jalur Swasta gagal dilakukan.";
                return json_encode($response);
            }
        }
    }
