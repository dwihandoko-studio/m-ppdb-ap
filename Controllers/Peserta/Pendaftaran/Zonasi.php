        ...
        if ($cekRegisterTemp) {
            ...
            if ($cekRegisterApprove) {
                if ($cekRegisterApprove->status_pendaftaran == 2) {
                    $data['success'] = "Anda dinyatakan <b>LOLOS</b> pada seleksi PPDB Tahun Ajaran 2023/2024 di :<br/><b>" . getNamaAndNpsnSekolah($cekRegisterApprove->tujuan_sekolah_id_1) . "</b> Melalui Jalur " . $cekRegisterApprove->via_jalur . ". <br/>Selanjutnya silahkan melakukan konfirmasi dan daftar ulang ke Sekolah Tujuan <br>sesuai jadwal yang telah ditentukan.";
                }
        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                try {
                    ...
                    $riwayatLib->insert("Mendaftar Melalui Jalur Zonasi, untuk diverifikasi berkas oleh sekolah tujuan. ", "Daftar Jalur Zonasi");
                    // $riwayatLib->insert("Mendaftar Melalui Jalur Zonasi, dengan No Pendaftaran : " . $data['kode_pendaftaran'], "Daftar Jalur Zonasi");
                } catch (\Throwable $th) {
                }
                $response = new \stdClass;
                $response->code = 200;
                $response->data = $data;
                $response->message = "Pendaftaran Melalui Jalur Zonasi berhasil dilakukan.";
                $response->redirrect = base_url('peserta/home');
                // $response->message = "Pendaftaran Melalui Jalur Zonasi berhasil dilakukan. Kode pendaftaran anda : " . $data['kode_pendaftaran'] . ". Selanjutnya Silahkan cetak bukti pendaftaran anda.";
                return json_encode($response);
            } else {
                $this->_db->transRollback();
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Pendaftaran Melalui Jalur Zonasi gagal dilakukan.";
                return json_encode($response);
            }
