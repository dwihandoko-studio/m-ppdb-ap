...
class Prestasi extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $where = "a.tujuan_sekolah_id_1 = '$getCurrentUser->sekolah_id' AND a.via_jalur = 'PRESTASI' AND (a.status_pendaftaran = 0)";
        ...
        if ($data['countData'] > 0) {
            if (count($data['result']) > 0) {
                ...
                $response->data = view('sekolah/pendaftaran/prestasi/pilihan-zonasi', $data);
                $response->pagination = view('sekolah/pendaftaran/prestasi/pilihan-pagination', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->code = 204;
    ...
    {
        $data['title'] = 'PENDAFTAR JALUR PRESTASI';
        $Profilelib = new Profilelib();
        $user = $Profilelib->userSekolah();
        if ($user->code != 200) {
            delete_cookie('jwt');
        ...
        return view('sekolah/pendaftaran/prestasi/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('sekolah/pendaftaran/prestasi/detail', $data);
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        $riwayatLib->insert("Memverifikasi Pendaftaran $name Melalui Jalur Prestasi dengan No Pendaftaran : " . $cekRegisterTemp['kode_pendaftaran'], "Memverifikasi Pendaftaran Jalur Prestasi", "submit");
        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        $riwayatLib = new Riwayatlib();
                        $riwayatLib->insert("Menolak Pendaftaran $name Melalui Jalur Prestasi dengan NISN : " . $nisn, "Tolak Pendaftaran Jalur Prestasi", "tolak");
