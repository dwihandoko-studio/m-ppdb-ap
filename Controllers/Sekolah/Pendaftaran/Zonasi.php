...
class Zonasi extends BaseController
{
    ...
    public function getAll()
    {
        ...
        if ($data['countData'] > 0) {
            if (count($data['result']) > 0) {
                ...
                $response->data = view('sekolah/pendaftaran/zonasi/pilihan-zonasi', $data);
                $response->pagination = view('sekolah/pendaftaran/zonasi/pilihan-pagination', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->code = 204;
    ...
    {
        $data['title'] = 'PENDAFTAR JALUR ZONASI';
        $Profilelib = new Profilelib();
        $user = $Profilelib->userSekolah();
        if ($user->code != 200) {
            delete_cookie('jwt');
        ...
        return view('sekolah/pendaftaran/zonasi/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('sekolah/pendaftaran/zonasi/detail', $data);
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        $saveNotifSystem->send([
                            ...
                            'isi' => "Pendaftaran anda melalui jalur zonasi ditolak dengan keterangan: $keterangan.",
                            'action_web' => 'peserta/riwayat/pendaftaran',
                            'action_app' => 'riwayat_pendaftaran_page',
                            'token' => $cekRegisterTemp['id'],
                            'send_from' => $user->data->id,
                        ...
                        $send = $onesignal->pushNotifToUser([
                            ...
                            'content' => "Pendaftaran anda melalui jalur zonasi ditolak dengan keterangan: $keterangan.",
                            'send_to' => $cekRegisterTemp['user_id'],
                            'app_url' => 'riwayat_pendaftaran_page',
                        ]);
                    } catch (\Throwable $th) {
