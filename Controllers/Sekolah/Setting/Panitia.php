...
class Panitia extends BaseController
{
    ...
    public function add()
    {
        ...
        $sekolahId = $this->_db->table('_users_profil_tb a')
            ...
            ->where('a.id', $user->data->id)->get()->getRowObject();
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_setting_panitia_tb')
    ...
    public function hapus()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_setting_panitia_tb')->where('id', $id)->get()->getRowObject();
            ...
            if ($current) {
                ...
                if ((int)$current->is_locked == 1) {
                    $response = new \stdClass;
                    $response->code = 400;
                    $response->message = "Settingan Panitia sudah terkunci. Untuk merubah silahkan hubungi admin paniti PPDB Dinas.";
                    return json_encode($response);
                }
                $this->_db->table('_setting_panitia_tb')->where('id', $id)->delete();

                if ($this->_db->affectedRows() > 0) {
                    try {
                        $riwayatLib = new Riwayatlib();
                        $riwayatLib->insert("Menghapus setting panitia sekolah", "Menghapus Panitia Sekolah", "delete");
                    } catch (\Throwable $th) {
                    }
                    $response = new \stdClass;
                    $response->code = 200;
                    $response->message = "Data panitia berhasil dihapus.";
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->code = 400;
    ...
    public function editSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $oldData = $this->_db->table('_setting_panitia_tb')->where('id', $id)->get()->getRowObject();
            if (!$oldData) {
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Data tidak ditemukan.";
            ...
            if ($nama == $oldData->nama && $nohp == $oldData->no_hp) {
                $response = new \stdClass;
                $response->code = 200;
            ...
            $cekData = $this->_db->table('_setting_panitia_tb')->where(['sekolah_id' => $oldData->sekolah_id, 'nama' => $nama, 'no_hp' => $nohp])->get()->getRowObject();

            if ($cekData) {
                $response = new \stdClass;
                $response->code = 400;
                $response->message = "Panitia ini untuk sekolah anda sudah di set.";
                return json_encode($response);
            }
            ...
            try {
                $this->_db->table('_setting_panitia_tb')->where('id', $oldData->id)->update($data);
                if ($this->_db->affectedRows() > 0) {
                    $this->_db->transCommit();
                    try {
                        $riwayatLib = new Riwayatlib();
                        $riwayatLib->insert("Mengedit setting panitia sekolah", "Mengedit Panitia Sekolah", "update");
                    } catch (\Throwable $th) {
                    }
                    $response = new \stdClass;
                    $response->code = 200;
