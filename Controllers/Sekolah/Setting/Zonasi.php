...
class Zonasi extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('sekolah/setting/zonasi/index', $data);
    }
        ...
        if ($jwt) {
            ...
            try {
                ...
                if ($decoded) {
                    ...
                    $response->data = view('sekolah/setting/zonasi/add', $data);
                    return json_encode($response);
                } else {
                    delete_cookie('jwt');
                    session()->destroy();
                ...
                try {
                    ...
                    if ($decoded) {
                        ...
                        if ($current) {
                            ...
                            $response->code = 200;
                            $response->message = "Permintaan diizinkan";
                            $response->data = view('sekolah/setting/zonasi/edit', $data);
                            return json_encode($response);
                        } else {
                            $response = new \stdClass;
                            $response->code = 400;
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->code = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('sekolah/setting/zonasi/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->code = 400;
    ...
    public function addSave()
    {
        ...
        if (!$this->validate($rules)) {
            $response = new \stdClass;
            $response->code = 400;
            $response->message = $this->validator->getError('prov') . $this->validator->getError('kab') . $this->validator->getError('kec') . $this->validator->getError('kel') . $this->validator->getError('dusun');
            return json_encode($response);
        } else {
