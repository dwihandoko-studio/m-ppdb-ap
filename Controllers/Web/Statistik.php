...
class Statistik extends BaseController
{
    ...
    public function index()
    {
        ...
        $detail = $this->_db->table('ref_sekolah a')
            ->select("a.id, ((SELECT count(id) FROM _tb_pendaftar_temp WHERE via_jalur = 'ZONASI') + (SELECT count(id_pendaftaran) FROM _tb_pendaftar_zonasi WHERE via_jalur = 'ZONASI')) as zonasi, ((SELECT count(id) FROM _tb_pendaftar_temp WHERE via_jalur = 'AFIRMASI') + (SELECT count(id_pendaftaran) FROM _tb_pendaftar_zonasi WHERE via_jalur = 'AFIRMASI')) as afirmasi, ((SELECT count(id) FROM _tb_pendaftar_temp WHERE via_jalur = 'MUTASI') + (SELECT count(id_pendaftaran) FROM _tb_pendaftar_zonasi WHERE via_jalur = 'MUTASI')) as mutasi, ((SELECT count(id) FROM _tb_pendaftar_temp WHERE via_jalur = 'PRESTASI') + (SELECT count(id_pendaftaran) FROM _tb_pendaftar_zonasi WHERE via_jalur = 'PRESTASI')) as prestasi, ((SELECT count(id) FROM _tb_pendaftar_temp WHERE via_jalur = 'SWASTA') + (SELECT count(id_pendaftaran) FROM _tb_pendaftar_zonasi WHERE via_jalur = 'SWASTA')) as swasta, ((SELECT count(id) FROM _tb_pendaftar_temp) + (SELECT count(id_pendaftaran) FROM _tb_pendaftar_zonasi)) as total ")
            ->limit(1)
            ->get()
            ->getRowObject();
    ...
    public function getPendaftaranSekolah()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($lists as $list) {
                ...
                // $row['npsn'] = $list->npsn;
                $row['nama'] = '<div style="font-size: 13px; vertical-align: inherit;">' . $list->nama_sekolah . '<br/>' . $list->npsn . '<br/>' . $list->nama_kecamatan . '<br/><b>Total Kuota : ' . ($list->zonasi + $list->afirmasi + $list->mutasi + $list->prestasi) . '</b><br/><b>Total Diterima : ' . ($list->terverifikasi_zonasi + $list->terverifikasi_afirmasi + $list->terverifikasi_mutasi + $list->terverifikasi_prestasi + $list->terverifikasi_swasta) . '</b><br/><b>' . (($list->zonasi + $list->afirmasi + $list->mutasi + $list->prestasi) - ($list->terverifikasi_zonasi + $list->terverifikasi_afirmasi + $list->terverifikasi_mutasi + $list->terverifikasi_prestasi + $list->terverifikasi_swasta)) . '</b></div>';
