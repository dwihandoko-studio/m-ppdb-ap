    ...
    public function getPendaftaranSekolah()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($lists as $list) {
                ...
                $row['nama'] = '<div style="font-size: 13px; vertical-align: inherit;">' . $list->nama_sekolah . '<br/>' . $list->npsn . '<br/><b>Total Kuota : ' . ($list->zonasi + $list->afirmasi + $list->mutasi + $list->prestasi) . '</b><br/><b>Total Diterima : ' . ($list->diterima_zonasi + $list->diterima_afirmasi + $list->diterima_mutasi + $list->diterima_prestasi + $list->diterima_swasta) . '</b><br/><b>Sisa Kuota : ' . (($list->zonasi + $list->afirmasi + $list->mutasi + $list->prestasi) - ($list->diterima_zonasi + $list->diterima_afirmasi + $list->diterima_mutasi + $list->diterima_prestasi + $list->diterima_swasta)) . '</b></div>';
