...
class Tokenlib
{
    ...
    {
        $cekUser = $this->_db->table('_users_tb')->orWhere(['id' => $id, 'email' => $id])->get()->getRowObject();
        if (!$cekUser) {
            $response = new \stdClass;
            $response->code = 401;
            $response->message = "User tidak diketahui.";
            return $response;
        }

        $user = $this->tb_token->where("(user_id = '{$cekUser->id}') AND token = '$token'")->orderBy('id', 'DESC')->limit(1)->get()->getRowObject();
        ...
        if ($user) {
            ...
            $enddate = strtotime("+1 week", strtotime($user->created_at));
            ...
            if ($today < $enddate) {
                ...
                $response = new \stdClass;
                $response->code = 401;
                $response->message = "Token telah expired.";
                return $response;
            }
        } else {
            $response = new \stdClass;
