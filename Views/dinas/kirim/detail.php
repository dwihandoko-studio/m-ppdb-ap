...
?>
        <form id="formAddData" class="form-horizontal form-add-data" method="post">
            <input type="hidden" value="<?= $data->id_pendaftaran ?>" name="_id_pendaftaran" />
        ...
        <script>
            ...
            function actionDaftarPelimpahan(event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        const id_pendaftaran = document.getElementsByName('_id_pendaftaran')[0].value;
                        
                        $.ajax({
                            url: BASE_URL + '/dinas/kirim/ubahpelimpahan',
                            type: 'POST',
                            data: {
            ...
            function changeKecamatan(event) {
                if (event.value !== "") {
                    ...
                    $.ajax({
                        url: BASE_URL + '/dinas/kirim/getSekolah',
                        type: 'POST',
                        data: {
                            id: event.value,
                        },
